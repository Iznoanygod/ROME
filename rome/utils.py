"""Model loading and weight-reload utilities.

`load_model` constructs the model + tokenizer from a `ModelConfig`. After
training, weights are persisted via `save_model` and picked up by long-lived
generator tasks in-place via `reload_model` (which dispatches to
`reload_lora` for PEFT adapters). Generators discover that new weights are
available by polling the integer weight-version counter that the trainer
bumps after `save_model` — see `bump_weight_version` / `read_weight_version`.
"""
import os
